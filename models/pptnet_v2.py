...
from models import pt_util

__all__ = ['Network']

class PPTparams(object):
    def __init__(self):
      self.FEATURE_SIZE = [256,256,256,256]
      self.MAX_SAMPLES = [64,256,1024,4096]
      self.CLUSTER_SIZE = [1,4,16,64]
      self.OUTPUT_DIM = [256,256,256,256]  #  [128,128,128,128][32,32,32,32] 
      self.GATING = True
      self.AGGREGATION = 'spvlad'
      self.SAMPLING = [1024,256,64,16]
      self.KNN = [20,20,20,20]
      self.GROUP = 8
      self.norm = True
...
class NetworkLAWS(nn.Module):
    def __init__(self, param=None):
        super(NetworkLAWS, self).__init__()
        # backbone
        self.backbone = PointNet2(param=param)
        self.param = param
        self.curr_group = 0
    ...
    def update_aggregators(self):
        aggregation = lp.SpatialPyramidNetVLAD(
                feature_size=self.param.FEATURE_SIZE,         # 256,256,256,256
                max_samples=self.param.MAX_SAMPLES,           # 64,256,1024,4096
                cluster_size=self.param.CLUSTER_SIZE,         # 1,4,16,64
                output_dim=self.param.OUTPUT_DIM,             # 256,256,256,256
                gating=self.param.GATING,                     # True
                add_batch_norm=True
            )
        self.aggregators.append(aggregation)
    
    def feature_extraction_training_module(self, f0, f1, f2, f3):
        """
        f0, B 256 4096 1
        f1, B 256 1024 1
        f2, B 256 2056 1
        f3, B 256 64 1
        """
        x = self.aggregators[self.curr_group](f0, f1, f2, f3)   # B x C0x64x1, BxC1x256, BxC2x1024, BxC3x4096 -> Bx256
        
        x = F.normalize(x, p=2, dim=1)

        return x
    
    def feature_extraction_inference_module(self, f0, f1, f2, f3):
        """
        f0, B 256 4096 1
        f1, B 256 1024 1
        f2, B 256 2056 1
        ...
        for aggregator in self.aggregators:
            x = aggregator(f0, f1, f2, f3)   # B x C0x64x1, BxC1x256, BxC2x1024, BxC3x4096 -> Bx256
    ...
    def forward(self, x, is_training=True):
        ...
        # x = F.normalize(x, p=2, dim=1)
        if is_training:
          x = self.feature_extraction_training_module(f0, f1, f2, f3)
        else:
          x = self.feature_extraction_inference_module(f0, f1, f2, f3)
...
class PointNet2(nn.Module):
    ...
    def forward(self, pointcloud: torch.cuda.FloatTensor):
        ...
        if pointcloud.shape[-1] == 6:
           l_xyz, l_features = [pointcloud[:,:,:3].contiguous()], [pointcloud[:,:,3:].transpose(1, 2).contiguous()]
        elif pointcloud.shape[-1] == 3:
