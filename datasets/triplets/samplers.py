...
class BatchSampler(Sampler):
    ...
    def __init__(self, dataset: TrainingDataset, batch_size: int, batch_size_limit: int = None,
        ...
        # self.elems_ndx = list(self.dataset.files)    # List of point cloud indexes
        self.elems_ndx = list(range(0, len(self.dataset.files)))
    def __iter__(self):
    ...
    def generate_batches(self):
        ...
        while True:
            ...
            selected_element_drive_id, selected_element_query_id = self.dataset.files[selected_element][0], self.dataset.files[selected_element][1]
            positives = self.dataset.get_positives(selected_element_drive_id, selected_element_query_id)
            if len(positives) == 0:
