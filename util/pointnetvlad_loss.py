...
def quadruplet_loss(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2, use_min=False, lazy=False, ignore_zero_loss=False, soft_margin=False):
    ...
    # positive = positive.view(-1, 1)
    positive = positive.repeat(int(num_neg))

    loss = m1 + positive - ((neg_vecs - query_copies)** 2).sum(1)
    ...
    second_loss = m2 + positive - ((neg_vecs - other_neg_copies)** 2).sum(1)
    if soft_margin:
    ...
    if lazy:
        ...
        second_loss = second_loss.mean(0)
    if ignore_zero_loss:
...
def val_loss(q_vec, pos_vecs, neg_vecs, other_neg, m1, m2, use_min=False, lazy=False, ignore_zero_loss=False, soft_margin=False):
    min_pos, max_pos = best_pos_distance(q_vec, pos_vecs)
    ...
    pos = positive.mean()
    neg = (((neg_vecs - query_copies)** 2).sum(2)).mean()
    if soft_margin:
        loss = loss.clamp(max=88)
        loss = torch.log(1 + torch.exp(loss))   # softplus
    else:
